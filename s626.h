/*
 ksrc/drivers/analogy/sensoray/s626.h
 Sensoray s626 Comedi driver, header file

 Xenomai Analogy driver for Sensoray 626 based
 on Comedi driver

 Copyright (C) 2000 Wojciech Domski <Wojciech.Domski@gmail.com>
...
/*  LP_GSEL setting for 5V bipolar range. */
#define GSEL_BIPOLAR10V		0x00A0
/*  LP_GSEL setting for 10V bipolar range. */

/*  Error codes that must be visible to this base class. */
#define ERR_ILLEGAL_PARM	0x00010000
/*  Illegal function parameter value was specified. */
#define ERR_I2C			0x00020000
/*  I2C error. */
#define ERR_COUNTERSETUP	0x00200000
/*  Illegal setup specified for counter channel. */
#define ERR_DEBI_TIMEOUT	0x00400000
/*  DEBI transfer timed out. */

/*  Organization (physical order) and size (in DWORDs)
 * of logical DMA buffers contained by ANA_DMABUF. */
#define ADC_DMABUF_DWORDS	40
/*  ADC DMA buffer must hold 16 samples, plus pre/post garbage samples. */
#define DAC_WDMABUF_DWORDS	1
/*  DAC output DMA buffer holds a single sample. */
...
/*  GPIO 0,2,3 = inputs, GPIO3 = IRQ; GPIO1 = out. */
...
/*  Audio output DMA2 protection address reached. */
#define PSR_AFOU		0x00000800
/*  Audio FIFO under/overflow detected. */
#define PSR_GPIO2		0x00000020
/*  GPIO2 input pin: 0=AdcBusy, 1=AdcIdle. */
#define PSR_EC0S		0x00000001
/*  Event counter 0 threshold reached. */

/*  Secondary Status Register (SSR) constants. */
#define SSR_AF2_OUT		0x00000200
/*  Audio 2 output FIFO under/overflow detected. */

/*  Master Control Register 1 (MC1) constants. */
#define MC1_SOFT_RESET		0x80000000
/*  Invoke 7146 soft reset. */
#define MC1_SHUTDOWN		0x3FFF0000
/*  Shut down all MC1-controlled enables. */
...
#define I2CR			(devpriv->i2c_adrs | 1)

/*  I2C write command | EEPROM address. */
#define I2CW			(devpriv->i2c_adrs)
...
#define ADDR_REG(reg) \
	(s626ptr->private->io_base + (reg))
#define ADDR_CHAN_REG(reg, chan) \
	(s626ptr->private->io_base + (reg) + (chan) * 8)
...
struct enc_private {
	...
	uint16_t (*get_enable)(struct s626_struct *s626ptr,
		struct enc_private *);
	/* Return clock enable. */
	uint16_t (*get_int_src)(struct s626_struct *s626ptr,
		struct enc_private *);
	/* Return interrupt source. */
	uint16_t (*get_load_trig)(struct s626_struct *s626ptr,
		struct enc_private *);
	/* Return preload trigger source. */
	uint16_t (*get_mode)(struct s626_struct *s626ptr, struct enc_private *);
	/* Return standardized operating mode. */
	void (*pulse_index)(struct s626_struct *s626ptr, struct enc_private *);
	/* Generate soft index strobe. */
	void (*set_enable)(struct s626_struct *s626ptr, struct enc_private *,
		uint16_t enab);
	/* Program clock enable. */
	void (*set_int_src)(struct s626_struct *s626ptr, struct enc_private *,
		uint16_t int_source);
	/* Program interrupt source. */
	void (*set_load_trig)(struct s626_struct *s626ptr, struct enc_private *,
		uint16_t trig);
	/* Program preload trigger source. */
	void (*set_mode)(struct s626_struct *s626ptr, struct enc_private *,
		uint16_t setup, uint16_t disable_int_src);
	/* Program standardized operating mode. */
	void (*reset_cap_flags)(struct s626_struct *s626ptr,
		struct enc_private *);
	...
	uint16_t my_latch_lsw;
	/*    Address of Latch least-significant-word */
	/*    register. */
	uint16_t my_event_bits[4];
...
struct counter_mode_register_t {
	...
	unsigned short cout_source:1;
	unsigned short cout_polarity:1;
	unsigned short auto_load_reset_rcap:3;
	unsigned short hw_ct_enable_source:2;
	unsigned short ct_enable_ctrl:2;
	unsigned short clock_source:2;
	unsigned short count_dir:1;
	unsigned short count_dir_ctrl:1;
	unsigned short output_reg_latch_ctrl:1;
	unsigned short preload_reg_sel:1;
...
static uint16_t get_enable_a(struct s626_struct *s626ptr,
	struct enc_private *k);

static uint16_t get_enable_b(struct s626_struct *s626ptr,
	struct enc_private *k);
...
static void reset_dac(struct s626_struct *s626ptr, uint8_t *ppl,
	a4l_cmd_t *cmd);
